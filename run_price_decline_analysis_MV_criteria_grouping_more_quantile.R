...
# example: run_price_decline_analysis_MV_criteria_grouping_more_quantile(63, "v_SecuritizedPricingLiquidity_202410231158_CLO.csv" )

run_price_decline_analysis_MV_criteria_grouping_more_quantile<- function(risk_window,data_file_name)
{
  ...
  merged_data[, `Rating Bucket` := sapply(`Orig Rating`, bucket_rating_MV_criteria)]
  ...
  merged_data_rating_bucket[, `Orig Rating` := sapply(`Orig Rating`, bucket_rating_MV_criteria)]
  ...
  output_file_path <- paste0("../report/result_quantile_",asset_name_with_date,"_with_MV_criteria_grouping_",risk_window,"_days",".xlsx")
  #result_wb <- createWorkbook()
  
  if (file.exists(output_file_path)) {
    # load the existing workbook
    result_wb <- loadWorkbook(output_file_path)
  } else {
  ...
  #writing main output to a new tab in the result workbook
  if (length(result_wb$sheet_names) > 0) {
    if (!asset_name %in% result_wb$sheet_names) {
  ...
  # add another new worksheet for the worst information
  worst_tab_name <- paste0(asset_name, "_WORST")
  if (length(result_wb$sheet_names) > 0) {
    if (!worst_tab_name %in% result_wb$sheet_names) {
      addWorksheet(result_wb, worst_tab_name)
      writeData(result_wb, worst_tab_name, worst_info_df, rowNames = TRUE, keepNA = TRUE)
    } 
  } else{
    #result_wb <- loadWorkbook(output_file_path)
    addWorksheet(result_wb, asset_name)
  ...
  #saveWorkbook(result_wb, paste0("../report/result_quantile_",asset_name_with_date,"_with_MV_criteria_grouping_",risk_window,"_days",".xlsx"), overwrite = TRUE)
  saveWorkbook(result_wb, output_file_path, overwrite = TRUE)
